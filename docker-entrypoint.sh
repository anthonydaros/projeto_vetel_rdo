...
until mysql -h"${DB_HOST:-mysql}" -P"${DB_PORT:-3306}" -u"${DB_USERNAME:-root}" -p"${DB_PASSWORD:-root}" -e "SELECT 1" >/dev/null 2>&1; do
...
TABLE_COUNT=$(mysql -h"${DB_HOST:-mysql}" -P"${DB_PORT:-3306}" -u"${DB_USERNAME:-root}" -p"${DB_PASSWORD:-root}" -D"${DB_DATABASE:-vetel_db}" -e "SELECT COUNT(*) FROM information_schema.tables WHERE table_schema='${DB_DATABASE:-vetel_db}';" -s -N)

if [ "$TABLE_COUNT" -eq 0 ]; then
    echo "Database is empty. Running initialization script..."
    mysql -h"${DB_HOST:-mysql}" -P"${DB_PORT:-3306}" -u"${DB_USERNAME:-root}" -p"${DB_PASSWORD:-root}" -D"${DB_DATABASE:-vetel_db}" < /var/www/html/sql/formulario_bd.sql
